...
def install_human_linux(node, user, control_ipv4_addr, password, cloud_config):
    ...
    packages = 'python3 python3-pip virtualenv xvfb unzip build-essential git autotools-dev autoconf libncursesw5-dev libtool autoconf automake bison flex libevent-dev ncurses-dev golang-go ninja-build gettext libtool libtool-bin autoconf automake cmake g++ pkg-config curl doxygen gnutls-dev libgnutls28-dev pkg-config build-essential groff-base libpipeline-dev libgdbm-dev groff libtool m4 xz-utils lzip'
    cmd = f"""
        set -x
